...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        return ((self.coef_wlk_1 * self.weight + (self.get_mean_speed()
                ** 2 // self.height) * self.coef_wlk_2 * self.weight)
                * self.duration * 60)


class Swimming(Training):
    """Тренировка: плавание."""
    coef_swem_1: float = 1.1
    coef_swem_2: float = 2
    LEN_STEP: float = 1.38
    ...
    def get_spent_calories(self) -> float:
        ...
        return ((self.get_mean_speed() + self.coef_swem_1)
                * self.coef_swem_2 * self.weight)
